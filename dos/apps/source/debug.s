; DEBUG.S
; MODIFY A .SAV RECORD
; GAVIN TERSTEEG, 2024
; SDMAY24-14

; BANK ALLOCATION STUFF
BI	= 1
BD	= 1

; MAXIMUM ARGUMENTS
MAXARGS	= 1

.TEXT
.BANK	BI
CORE0_B	= BI

	; START BY PROCESSING THE ARGUMENTS
START:	LOADI	A,0
	STORE	[DBANK],A
	
	; RESET ARG STATE
	STORE	[ARGC],A
	
	; SET UP STACK
	LOADI	D,0X60-2
	
	; SYSCALL RETURN BANK
	LOADI	C,1	
	STOREF	[D+1],C
	
	; SET UP POINTER
	LOADI	C,0
	
	; GO TO ARGUMENT BANK
0:	LOAD	A,[CMDL_B]
	STORE	[ARG_BNK],A
	STORE	[DBANK],A

	; SKIP THE CURRENT ARGUMENT
1:	LOADF	A,[C]
	LOADI	B,0X20
	CMP	A,B
	BRBE	2F
	ADDI	C,1
	JUMP	1B

	; LOOK FOR AN ARGUMENT
2:	LOADF	A,[C]
	ADDI	A,0
	BRZ	ARGDONE
	CMP	A,B
	BRA	3F
	ADDI	C,1
	JUMP	2B

	; IS IT A FLAG?
3:	LOADI	B,'-'
	CMP	A,B
	BRNZ	5F
	
	; HANDLE FLAGS HERE	
4:	ADDI	C,1
	LOADF	A,[C]
	ADDI	A,0
	BRZ	ARGDONE
	LOADI	B,0X20
	CMP	A,B
	BRBE	2B
	
	; REGISTER THE FLAG
	LOADI	B,0
	STORE	[DBANK],B

;	SUBI	A,'A'
;	BRNZ	@+2
;	LOADI	B,AFLAG
	
	; IS IT A RECOGNIZED FLAG?
	ADDI	B,0
	BRZ	ARGBAD
	LOADI	A,1
	STOREF	[B],A

	; THERE MAY BE ANOTHER FLAG
	LOAD	B,[CMDL_B]
	STORE	[ARG_BNK],B
	STORE	[DBANK],B
	JUMP	4B

	; SAVE THE ARGUMENT
5:	LOADI	B,0
	STORE	[DBANK],B
	LOAD	B,[ARGC]
	STOREF	[B+ARGV],C
	
	; DID WE EXCEED THE ALLOWED NUMBER OF ARGUMENTS
	SUBI	B,MAXARGS
	BRC	ARGBAD
	ADDI	B,MAXARGS+1
	STORE	[ARGC],B
	JUMP	0B
	
	; BAD ARGUMENT
ARGBAD:	LOADI	A,0
	STORE	[DBANK],A
	
	LOADI	A,STR0_B
	STORE	[ARG_BNK],A
	LOADI	A,ERROR0
	
	; PRINT ERROR MESSAGE
	LOADI	B,S_PUTS
	LOADI	C,EXIT
	JUMP	SYSCALL	

	; EXIT PROGRAM
EXIT:	LOADI	B,0
	JUMP	SYSJUMP
	
	; ARGUMENT PROCESSING DONE
ARGDONE:LOADI	A,0
	STORE	[DBANK],A
	
	; CHECK ARG COUNT
	LOAD	A,[ARGC]
	SUBI	A,1
	BRNZ	ARGBAD
	
	; CALL SVREAD
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,FIO_B
	LOADI	C,SVREAD
	JUMP	INDIR
	
	; GO TO PROMPT
	LOADI	B,CMD0_B
	LOADI	C,PROMPT
	JUMP	INDIR


	; BANK IS DONE, MOVE ON TO THE NEXT
BI	= BI+1
.TEXT
.BANK	BI
CMD0_B	= BI

ERROR:	LOADI	A,ST_ERR
	JUMP	0F

	; SEND UP THE COMMAND LINE PROMPT
PROMPT:	LOADI	A,ST_PRM
0:	LOADI	C,BI
	STOREF	[D+1],C
	LOADI	B,STR0_B
	STORE	[ARG_BNK],B
	LOADI	B,S_PUTS
	LOADI	C,@+2
	JUMP	SYSCALL
	
	; GET USER INPUT
	LOADI	B,BUF_B
	STORE	[ARG_BNK],B
	LOADI	B,S_INPUT
	LOADI	C,@+2
	JUMP	SYSCALL

	; PREPARE TO PARSE
	LOADI	A,0
	STORE	[POINTER],A
	STORE	[STATE],A
	STORE	[ADDRESS],A
	
	; EXECUTE PARSE
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,CMD0_B
	LOADI	C,PARSEH
	JUMP	INDIR
	ADDI	A,0
	BRN	ERROR
	
	; MAKE SURE THE NEXT VALUE IS ZERO
	LOAD	C,[POINTER]
	LOADI	B,BUF_B
	STORE	[DBANK],B
	LOADF	C,[C]
	LOADI	B,0
	STORE	[DBANK],B
	ADDI	C,0
	BRNZ	ERROR
	
	; PARSE COMMAND
	SUBI	A,'Q'
	BRNZ	1F
	LOAD	A,[STATE]
	ADDI	A,0
	BRNZ	ERROR
	LOADI	B,CMD3_B
	LOADI	C,QUIT
	JUMP	INDIR
	
	; MAKE SURE THAT VALUE IS LESS THAN 256
	; AND SET ADDRESS
1:	LOAD	B,[VALUE]
	ADDI	B,0
	BRNZ	ERROR
	LOAD	B,[VALUE+1]
	STORE	[ADDRESS],B
	
	; [M]ODIFY DATA MEMORY
	SUBI	A,'M'-'Q'
	LOADI	B,CMD1_B
	LOADI	C,MDATA
	BRZ	INDIR
	
	; MODIFY [I]NSTRUCTION
	SUBI	A,'I'-'M'
	LOADI	B,CMD2_B
	LOADI	C,MINSTR
	BRZ	INDIR
	
	; READ BLOCK
	SUBI	A,'R'-'I'
	LOADI	B,CMD3_B
	LOADI	C,RBANK
	BRZ	INDIR
	
	
	JUMP	ERROR
	
	; PARSES A HEXADECIMAL NUMBER
	; THE NUMBER WILL CONTINUE TO BE PARSED UNTIL
	; THE END OF THE NUMBER IS REACHED.
	; [POINTER] = ADDRESS OF NUMBER
	; RETURNS A=0XFF IF THE PARSING OVERFLOWED
	; USES: A, B, C
PARSEH:	LOADI	A,0
	STORE	[VALUE],A
	STORE	[VALUE+1],A

	; READ A CHARACTER FROM THE COMMAND LINE
0:	LOAD	C,[POINTER]
	LOADI	B,BUF_B
	STORE	[DBANK],B
	LOADF	A,[C]
	LOADI	B,0
	STORE	[DBANK],B
	ADDI	C,1
	STORE	[POINTER],C
	
	; CHECK BOUNDS
	LOADI	B,'0'
	CMP	A,B
	BRB	IRET
	LOADI	B,'9'
	CMP	A,B
	BRBE	1F
	
	LOADI	B,'A'
	CMP	A,B
	BRB	IRET
	LOADI	B,'F'
	CMP	A,B
	BRA	IRET
	
	SUBI	A,'A'-('0'+10)
	
	; CONVERT FROM ASCII
1:	SUBI	A,'0'

	; MULTIPLY VALUE BY 4
	STORE	[TEMP],A
	LOAD	B,[VALUE]
	LOAD	C,[VALUE+1]
	LOADI	A,4
2:	SHIFTL	B
	BRC	9F
	SHIFTL	C
	BRNC	3F
	ADDI	B,1
3:	SUBI	A,1
	BRNZ	2B
	
	; ADD TO VALUE
	LOAD	A,[TEMP]
	ADD	C,A
	STORE	[VALUE],B
	STORE	[VALUE+1],C
	
	; SET STATE
	LOADI	A,1
	STORE	[STATE],A
	JUMP	0B
	
	; ERROR
9:	LOADI	A,0XFF
	JUMP	IRET


	; BANK IS DONE, MOVE ON TO THE NEXT
BI	= BI+1
.TEXT
.BANK	BI
CMD1_B	= BI

	; MODIFY DATA
	; MAKE SURE STATE IS 1
MDATA:	LOAD	A,[STATE]
	ADDI	A,0
	BRZ	9F
	
	; MAKE SURE ADDRESS IS LESS THAN 128
	LOAD	A,[ADDRESS]
	LOADI	B,0X80
	CMP	A,B
	BRAE	9F
	
	; PRINT ADDRESS
0:	LOADI	C,BI
	STOREF	[D+1],C
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,PRINT_B
	LOADI	C,PADDR
	JUMP	INDIR
	
	; READ DATA AND PRINT
	LOAD	A,[ADDRESS]
	LOADI	B,WORK_B+1
	STORE	[DBANK],B
	LOADF	A,[A]
	LOADI	B,0
	STORE	[DBANK],B
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,PRINT_B
	LOADI	C,PBYTE
	JUMP	INDIR
	
	; PRINT COLON
	LOADI	A,':'
	LOADI	B,S_PUTC
	LOADI	C,@+2
	JUMP	SYSCALL
	
	; GET A LINE FROM THE USER
	LOADI	A,BUF_B
	STORE	[ARG_BNK],A
	LOADI	B,S_INPUT
	LOADI	C,@+2
	JUMP	SYSCALL
	
	; PARSE THE HEX
	LOADI	A,0
	STORE	[POINTER],A
	STORE	[STATE],A
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,CMD0_B
	LOADI	C,PARSEH
	JUMP	INDIR
	ADDI	A,0
	BRN	9F
	
	; SEE IF WE SHOULD EXIT
	ADDI	A,0
	BRNZ	8F
	
	; DO WE SKIP
	LOAD	A,[STATE]
	ADDI	A,0
	BRZ	1F
	
	; MAKE SURE VALUE IS LESS THAN 256
	LOAD	B,[VALUE]
	ADDI	B,0
	BRNZ	9F
	
	; STORE INTO MEMORY
	LOAD	A,[ADDRESS]
	LOAD	B,[VALUE+1]
	LOADI	C,WORK_B+1
	STORE	[DBANK],C
	STOREF	[A],B
	LOADI	C,0
	STORE	[DBANK],C
	LOADI	A,0XFF
	STORE	[DIRTY],A

	; INCREMENT ADDRESS
1:	LOAD	A,[ADDRESS]
	ADDI	A,1
	STORE	[ADDRESS],A
	BRNN	0B

	; NORMAL EXIT
8:	LOADI	B,CMD0_B
	LOADI	C,PROMPT
	JUMP	INDIR

	; ERROR EXIT
9:	LOADI	B,CMD0_B
	LOADI	C,ERROR
	JUMP	INDIR
	
	; BANK IS DONE, MOVE ON TO THE NEXT
BI	= BI+1
.TEXT
.BANK	BI
CMD2_B	= BI

	; MODIFY INSTRUCTION
	; MAKE SURE STATE IS 1
MINSTR:	LOAD	A,[STATE]
	ADDI	A,0
	BRZ	9F
	
	; MAKE SURE ADDRESS IS GREATER THAN 128
	LOAD	A,[ADDRESS]
	LOADI	B,0X80
	CMP	A,B
	BRB	9F
	
	; PRINT ADDRESS
0:	LOADI	C,BI
	STOREF	[D+1],C
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,PRINT_B
	LOADI	C,PADDR
	JUMP	INDIR
	
	; READ INSTRUCTION AND PRINT
	LOAD	A,[ADDRESS]
	LOADI	B,WORK_B+2
	SHIFTL	A
	SHIFTL	A
	BRNC	1F
	ADDI	B,1
1:	SHIFTR	A
	STORE	[DBANK],B
	LOADF	B,[A+1]
	LOADF	A,[A]
	LOADI	C,0
	STORE	[DBANK],C
	STORE	[HIGHB],B
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,PRINT_B
	LOADI	C,PBYTE
	JUMP	INDIR
	LOAD	A,[HIGHB]
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,PRINT_B
	LOADI	C,PBYTE
	JUMP	INDIR
	
	; PRINT COLON
	LOADI	A,':'
	LOADI	B,S_PUTC
	LOADI	C,@+2
	JUMP	SYSCALL
	
	; GET A LINE FROM THE USER
	LOADI	A,BUF_B
	STORE	[ARG_BNK],A
	LOADI	B,S_INPUT
	LOADI	C,@+2
	JUMP	SYSCALL
	
	; PARSE THE HEX
	LOADI	A,0
	STORE	[POINTER],A
	STORE	[STATE],A
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,CMD0_B
	LOADI	C,PARSEH
	JUMP	INDIR
	ADDI	A,0
	BRN	9F
	
	; SEE IF WE SHOULD EXIT
	ADDI	A,0
	BRNZ	8F
	
	; DO WE SKIP
	LOAD	A,[STATE]
	ADDI	A,0
	BRZ	3F
	
	; STORE INTO MEMORY
	LOAD	A,[ADDRESS]
	LOAD	B,[VALUE]
	LOADI	C,WORK_B+2
	SHIFTL	A
	SHIFTL	A
	BRNC	2F
	ADDI	C,1
2:	SHIFTR	A
	STORE	[DBANK],C
	STOREF	[A],B
	LOADI	B,0
	STORE	[DBANK],B
	LOAD	B,[VALUE+1]
	STORE	[DBANK],C
	STOREF	[A+1],B
	LOADI	C,0
	STORE	[DBANK],C
	LOADI	A,0XFF
	STORE	[DIRTY],A

	; INCREMENT ADDRESS
3:	LOAD	A,[ADDRESS]
	ADDI	A,1
	STORE	[ADDRESS],A
	BRN	0B

	; NORMAL EXIT
8:	LOADI	B,CMD0_B
	LOADI	C,PROMPT
	JUMP	INDIR

	; ERROR EXIT
9:	LOADI	B,CMD0_B
	LOADI	C,ERROR
	JUMP	INDIR

	; BANK IS DONE, MOVE ON TO THE NEXT
BI	= BI+1
.TEXT
.BANK	BI
CMD3_B	= BI


	; EXIT OUT OF PROGRAM
QUIT:	LOAD	A,[DIRTY]
	ADDI	A,0
	BRZ	9F
	
	; WRITE OUT THE CBLOCK
	LOADI	C,BI
	STOREF	[D+1],C
	LOADI	A,WORK_B
	STORE	[ARG_BNK],A
	LOAD	A,[CBLOCK]
	LOADI	B,S_WRITE
	LOADI	C,@+2
	JUMP	SYSCALL
	
	; EXIT
9:	LOADI	B,CORE0_B
	LOADI	C,EXIT
	JUMP	INDIR
	
	; READ A BANK
RBANK:	LOADI	C,BI
	STOREF	[D+1],C
	LOAD	A,[DIRTY]
	ADDI	A,0
	BRZ	9F
	
	; WRITE OUT THE CBLOCK
	LOADI	A,WORK_B
	STORE	[ARG_BNK],A
	LOAD	A,[CBLOCK]
	LOADI	B,S_WRITE
	LOADI	C,@+2
	JUMP	SYSCALL
	
	; ALLOCATE BLOCK ON MAP
	LOAD	C,[CBLOCK]
	LOADI	B,WORK_B
	STORE	[DBANK],B
	LOAD	A,[2]
	LOADI	B,BTAB_B
	SHIFTL	A
	BRNC	0F
	ADDI	B,1
0:	SHIFTR	A
	STORE	[DBANK],B
	STOREF	[A],C
	ADDI	B,2
	STORE	[DBANK],B
	LOADI	C,0XFF
	STOREF	[A],C
	LOADI	B,0
	STORE	[DBANK],B
	
	
	; READ IN A NEW BLOCK
9:	LOAD	A,[ADDRESS]
	LOADI	C,@+5
	STOREF	[D],C
	LOADI	B,BIO_B
	LOADI	C,BREAD
	JUMP	INDIR
	
	; GO BACK TO PROMPT
	LOADI	B,CMD0_B
	LOADI	C,PROMPT
	JUMP	INDIR

	; BANK IS DONE, MOVE ON TO THE NEXT
BI	= BI+1
.TEXT
.BANK	BI
FIO_B	= BI

	; READS IN THE INITIAL SAVE FILE
	; IF A RECORD IS FOUND, THE BLOCK WILL BE RECORDED
	; THE PURPOSE OF THIS IS TO GET A MAP OF WHAT BANKS
	; EXIST ON WHAT BLOCKS IN THE FILE
	; REBUILDS THE ALLOCATION TABLE
	; USES: A, B, C
SVREAD:	SUBI	D,2
	LOADI	A,0
	STORE	[ISFULL],A
	LOADI	C,0
0:	LOADI	B,ALLOC_B
	STORE	[DBANK],B
	STOREF	[C],A
	LOADI	B,ALLOC_B+1
	STORE	[DBANK],B
	STOREF	[C],A
	ADDI	C,1
	BRNN	0B
	STORE	[DBANK],A
	
	; ATTEMPT TO OPEN FILE
	LOAD	A,[CMDL_B]
	STORE	[ARG_BNK],A
	LOAD	A,[ARGV]
	LOADI	C,BI
	STOREF	[D+1],C
	LOADI	B,S_OPEN
	LOADI	C,@+2
	JUMP	SYSCALL
	ADDI	A,0
	BRZ	1F
	
	; CAN'T OPEN, CREATE THE FILE
	LOAD	A,[ARGV]
	LOADI	B,S_CREAT
	LOADI	C,@+2
	JUMP	SYSCALL
	ADDI	A,0
	BRZ	1F
	
	; ERROR
	LOADI	A,ERROR1
	JUMP	9F
	
	; RESET BLOCK COUNTER
1:	LOADI	A,WORK_B
	STORE	[ARG_BNK],A
	LOADI	C,0
	
	; READ A BLOCK
2:	STORE	[FNEXT],C
	MOV	A,C
	LOADI	B,S_READ
	LOADI	C,@+2
	JUMP	SYSCALL
	ADDI	A,0
	BRNZ	4F
	
	; CHECK BLOCK
	LOAD	C,[FNEXT]
	LOADI	A,ERROR3
	LOADI	B,WORK_B
	STORE	[DBANK],B
	LOAD	B,[0]
	SUBI	B,0X02
	BRNZ	9F
	LOAD	B,[1]
	SUBI	B,0X81
	BRNZ	9F
	
	; GRAB BANK #
	LOAD	A,[2]
	LOADI	B,BTAB_B
	SHIFTL	A
	BRNC	3F
	ADDI	B,1
3:	SHIFTR	A
	STORE	[DBANK],B
	STOREF	[A],C
	
	; MARK AS ALLOCATED
	ADDI	B,2
	STORE	[DBANK],B
	LOADI	B,0XFF
	STOREF	[A],B
	
	; INCREMENT BLOCK COUNTER
	LOADI	A,0
	STORE	[DBANK],A
	ADDI	C,1
	BRNC	2B
	LOADI	A,0XFF
	STORE	[ISFULL],A
	
	; DONE READING, LETS OPEN A BLOCK
4:	ADDI	D,2
	LOADI	A,1
	LOADI	B,BIO_B
	LOADI	C,BREAD
	JUMP	INDIR
	
	; ERROR OUT!
9:	LOADI	B,0
	STORE	[DBANK],B
	LOADI	B,CORE0_B
	STOREF	[D+1],B
	LOADI	B,STR0_B
	STORE	[ARG_BNK],B
	LOADI	B,S_PUTS
	LOADI	C,EXIT
	JUMP	SYSCALL
	
	; BANK IS DONE, MOVE ON TO THE NEXT
BI	= BI+1
.TEXT
.BANK	BI
BIO_B	= BI

	; READS IN A BANK
	; IF THAT BANK DOESN'T EXIST, CLEAR OUT THE BUFFER
	; DIRTY FLAG IS ALSO RESET
	; A = BANK TO READ IN
	; USES: A, B, C
BREAD:	SUBI	D,2
	
	; RESET DIRTY FLAG
	LOADI	B,0
	STORE	[DIRTY],B

	; SAVE BANK
	STORE	[TEMP],A

	; GRAB BANK #
	LOADI	B,ALLOC_B
	SHIFTL	A
	BRNC	3F
	ADDI	B,1
3:	SHIFTR	A
	STORE	[DBANK],B
	LOADF	C,[A]
	
	; CHECK IF IT'S BEEN ALLOCATED
	ADDI	C,0
	BRZ	0F
	
	; YEP, LETS READ IT INTO MEMORY
	SUBI	B,2
	STORE	[DBANK],B
	LOADF	A,[A]
	LOADI	C,0
	STORE	[DBANK],C
	STORE	[CBLOCK],A
	LOADI	C,BI
	STOREF	[D+1],C
	LOADI	C,WORK_B
	STORE	[ARG_BNK],C
	LOADI	B,S_READ 
	LOADI	C,@+2
	JUMP	SYSCALL
	ADDI	A,0
	BRZ	8F
	
	; CLEAR BUFFER
0:	LOADI	B,0
	STORE	[DBANK],B
	LOAD	A,[TEMP]
	LOADI	B,WORK_B
	STORE	[DBANK],B
	STORE	[2],A
	LOADI	A,0X02
	STORE	[0],A
	LOADI	A,0X81
	STORE	[1],A
	LOADI	A,0X03
	LOADI	B,0XFF
1:	STOREF	[A],B
	ADDI	A,1
	BRNN	1B
	
	LOADI	B,WORK_B+1
	STORE	[DBANK],B
	LOADI	A,0
	LOADI	C,0
2:	STOREF	[A],C
	ADDI	A,1
	BRNN	2B
	
	LOADI	B,WORK_B+2
	STORE	[DBANK],B
	LOADI	A,0
	LOADI	C,0X20
3:	STOREF	[A],C
	ADDI	A,1
	BRNN	3B
	
	LOADI	B,WORK_B+3
	STORE	[DBANK],B
	LOADI	A,0
4:	STOREF	[A],C
	ADDI	A,1
	BRNN	4B
	
	; CHECK IF THERE IS SPACE
	LOADI	A,0
	STORE	[DBANK],A
	LOAD	B,[ISFULL]
	LOADI	A,ERROR2
	ADDI	B,0
	BRNZ	9F
	
	; THERE IS SPACE, INCREMENT FNEXT
	LOAD	A,[FNEXT]
	STORE	[CBLOCK],A
	ADDI	A,1
	STORE	[FNEXT],A
	BRNZ	8F
	LOADI	A,0XFF
	STORE	[ISFULL],A

	; RESTORE STACK AND RETURN
8:	ADDI	D,2
	JUMP	IRET

	; ERROR OUT!
9:	LOADI	B,0
	STORE	[DBANK],B
	LOADI	B,CORE0_B
	STOREF	[D+1],B
	LOADI	B,STR0_B
	STORE	[ARG_BNK],B
	LOADI	B,S_PUTS
	LOADI	C,EXIT
	JUMP	SYSCALL
	
	
	; BANK IS DONE, MOVE ON TO THE NEXT
BI	= BI+1
.TEXT
.BANK	BI
PRINT_B	= BI

	; PRINT THE CURRENT ADDRESS
PADDR:	SUBI	D,2

	; DO A CR/LF
	LOADI	C,BI
	STOREF	[D+1],C
	LOADI	A,0X0A
	LOADI	B,S_PUTC
	LOADI	C,@+2
	JUMP	SYSCALL
	LOADI	A,0X0D
	LOADI	B,S_PUTC
	LOADI	C,@+2
	JUMP	SYSCALL

	; PRINT ADDRESS BYTE
	LOAD	A,[ADDRESS]
	LOADI	C,@+3
	STOREF	[D],C
	JUMP	PBYTE
	
	; PRINT SLASH AND SPACE
	LOADI	A,'/'
	LOADI	B,S_PUTC
	LOADI	C,@+2
	JUMP	SYSCALL

	ADDI	D,2 
	JUMP	IRET
	
	; PRINT A BYTE
	; A = BYTE TO PRINT
PBYTE:	SUBI	D,2
	STORE	[TEMP],A
	SHIFTR	A
	SHIFTR	A
	SHIFTR	A
	SHIFTR	A
	LOADI	C,BI
	STOREF	[D+1],C
	LOADI	C,@+3
	STOREF	[D],C
	JUMP	PNYBBLE
	ADDI	D,2
	LOAD	A,[TEMP]
	LOADI	B,16
0:	CMP	A,B
	BRB	PNYBBLE
	SUBI	A,16
	JUMP	0B
	
	; PUT A NYBBLE ON THE TERMINAL
	; A = NYBBLE TO OUTPUT
PNYBBLE:LOADI	B,10
	CMP	A,B
	BRB	0F
	ADDI	A,'A'-'0'-10
0:	ADDI	A,'0'
	LOADI	B,S_PUTC
	JUMP	SYSJUMP

.DATA
.BANK	BD
STR0_B	= BD

	; ERROR MESSAGES
.DEFL BYTE ERROR0	"INVALID ARGUMENTS",0X0A,0X0D,
			"USAGE: DEBUG FILE",0X0A,0X0D,0
.DEFL BYTE ERROR1	"CAN'T CREATE FILE",0X0A,0X0D,0
.DEFL BYTE ERROR2	"OUT OF SPACE",0X0A,0X0D,0
.DEFL BYTE ERROR3	"BAD RECORD",0X0A,0X0D,0

.DEFL BYTE ST_ERR	0X0A,0X0D,"?"
.DEFL BYTE ST_PRM	0X0A,0X0D,"@",0

	; ZERO BANK STUFF
.BANK	0
.BSS

	; COMMAND LINE ARGUMENTS
.DEFL BYTE ARGC		0
.DEFL BYTE ARGV		0,0

	; PARSE STATE
.DEFL BYTE POINTER	0
.DEFL BYTE STATE	0
.DEFL BYTE VALUE	0,0

	; BLOCK EDIT STATE
.DEFL BYTE CBLOCK	0
.DEFL BYTE DIRTY	0
.DEFL BYTE ADDRESS	0
.DEFL BYTE FNEXT	0
.DEFL BYTE ISFULL	0
.DEFL BYTE HIGHB	0

	; STACK PARKING SPACE
.DEFL BYTE SPARK	0

	; TEMPORARY VALUE
.DEFL BYTE TEMP		0

BD	= BD+1
.DATA
.BANK	BD
BUF_B	= BD

	; GENERAL PURPOSE BUFFER
	
BD	= BD+1
.DATA
.BANK	BD
BTAB_B	= BD
BD	= BD+1

	; BLOCK ADDRESS TABLE
	; TURNS A LOGIC SAV BLOCK INTO A PHYSICAL FILE BLOCK

BD	= BD+1
.DATA
.BANK	BD
ALLOC_B	= BD
BD	= BD+1

	; BLOCK ALLOCATION TABLE
	; MARKS IF A LOGICAL SAV BLOCK HAS BEEN ALLOCATED YET
	
BD	= BD+1
.DATA
.BANK	BD
WORK_B	= BD
BD	= BD+3

	; WORK BANK
	; WORK_B + 0 = RECORD HEADER
	; WORK_B + 1 = DATA SEGMENT
	; WORK_B + 2 = ISR SEGMENT 128-191
	; WORK_B + 3 = ISR SEGMENT 192-255