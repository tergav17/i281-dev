...
5:	LOADI	C,0
	...
	; A COUNTER MUST BE SUPPLIED TO DETERMINE
	; HOW MANY ENTRIES WILL BE SKIPPED BEFORE
	; A MATCH CAN BE MADE
	; A = NAME OF LOCAL SYMBOL
	; [CHAR] = DIRECTION (0 = BACKWARDS, 1 = FORWARDS)
	; [OPERAND] = # OF LOCALS TO SKIP
	; RETURNS A = 0XFF IF UNDEFINED, 0X00 OTHERWISE
	; VALUE IS RETURNED IN [VALUE]
LASTLOC	= TEMP
GETLOC:	STORE	[SPARK],D
	...
	LOADI	C,LT_NAME+(LT_SIZE-1)
	LOADI	D,LTAB_B
	
	; CHECK ENTRY
0:	STORE	[DBANK],D
	LOADF	B,[C+0-(LT_SIZE-1)]
	CMP	A,B
	LOADI	B,0
	STORE	[DBANK],B
	BRNZ	2F
	...
	; INDEX IS NOT ZERO, RECORD POINTER
1:	STORE	[LASTLOC],D
	STORE	[LASTLOC+1],C
